...
class HTTPCode403 extends HttpCode
{
	public function __construct($mdata)
	{
		if(get_class($mdata) !== 'HTTPMessageData')
			throw new Exception('Argument 1 to method HTTPCode403::__construct() not a class HTTPMessageData!');

		$t = NULL;
		parent::AddHeader('HTTP/1.1 403 Forbidden');
		...
		if(($t = $mdata -> GetMD5()) !== NULL)
			parent::AddHeader(HTTPEntityHeaders::Content-D5($t));
