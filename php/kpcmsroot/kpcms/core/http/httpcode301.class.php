...
class HTTPCode301 extends HttpCode
{
	public function __construct($mdata)
	{
		if(get_class($mdata) !== 'HTTPMessageData')
			throw new Exception('Argument 1 to method HTTPCode301::__construct() not a class HTTPMessageData!');
			
		$t = NULL;
		parent::AddHeader('HTTP/1.1 301 Moved Permanently');
		...
		parent::AddHeader(HTTPEntityHeaders::Content-Type($mdata -> GetMime()));
		...
		if(($t = $mdata -> GetRedirect()) !== NULL)
			parent::AddHeader(HTTPResponseHeaders::Location($t));
